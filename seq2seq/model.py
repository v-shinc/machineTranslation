...
def _shift(tensor):
    ...
    shape[0] = 1
    res = tf.concat(0, [tf.zeros(shape),  tensor[:-1]])
    return res
...
class Model:
    def __init__(self, params):
        ...
        x = tf.transpose(self.x, [0, 1])
        y = tf.transpose(self.y, [0, 1])
        x_mask = tf.transpose(self.x_mask, [0, 1])
        y_mask = tf.transpose(self.y_mask, [0, 1])
        ...
        x_emb = tf.nn.embedding_lookup(x_embedding, x)
        # pass through encoder gru, recurrence here
        self.hh = gru_layer(x_emb, x_mask, params['hidden_dim'])  # [seq_len, batch_size, hidden_dim]
        ...
        self.y_emb = tf.nn.embedding_lookup(y_embedding, y)
        ...
        with tf.variable_scope('decodert'):
            self.ss_ = cond_gru_layer(self.y_embed_shifted, y_mask, self.context, init_state, params['hidden_dim'])  # [seq_len, batch_size, hidden_dim]
        contexts = tf.expand_dims(self.context, 0)
        # compute word probabilities
        logit_lstm = ff_layer(self.ss_, params['word_dim'], 'ff_logit_lstm')
        logit_prev = ff_layer(tf.transpose(self.y_embed_shifted, [0, 1]), params['word_dim'], 'ff_logit_prev')
        ...
        # cost
        labels = tf.one_hot(y, params['target_vocab_size'])
        ...
        self.cost = - tf.reduce_mean(tf.reduce_sum(cost * y_mask, 0))
